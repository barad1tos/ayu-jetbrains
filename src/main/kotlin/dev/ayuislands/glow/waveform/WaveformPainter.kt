...
internal class WaveformRenderPlan internal constructor(
    internal val request: WaveformPaintRequest,
    val track: WaveformTrack,
    internal val strokes: WaveformPainter.SignalStrokes,
    internal val layers: WaveformPainter.SignalLayers?,
    internal val strength: Float,
    val signalBounds: Rectangle?,
)
...
private fun WaveformPaintRequest.snapshot(): WaveformPaintRequest =
    copy(
        ...
        frame = frame.copy(trace = frame.trace?.let { it.copy(history = it.history.toList()) }),
...
internal open class WaveformPainter(
    ...
    open fun prepare(request: WaveformPaintRequest): WaveformRenderPlan {
            ...
            SignalLayers(
                glow =
                    signalPaths(
                        ...
                        inwardEdges = snapshot.inwardEdges,
                    ),
                core =
                    signalPaths(
                        signal = signal,
        ...
        return WaveformRenderPlan(
            request = snapshot,
            track = track,
            strokes = strokes,
            layers = layers,
            strength = strength,
            signalBounds = signalBounds(layers, strokes, snapshot.bounds),
        )
    }
    ...
    private fun signalBounds(
        ...
        val paths =
            layers.glow.bands.asSequence() +
                layers.core.bands.asSequence() +
                layers.core.heads
                    .asSequence()
                    .map { it.path }
        for (path in paths) {
            if (path.currentPoint == null) continue
            val pathBounds = path.bounds
            bounds = bounds?.union(pathBounds) ?: pathBounds
        }
        val padded = bounds ?: return null
        val padding = ceil(strokes.outer / 2f).toInt() + SIGNAL_BOUNDS_PADDING
        padded.grow(padding, padding)
        val clipped = padded.intersection(overlayBounds)
    ...
    private fun signalPaths(
        ...
        appendSignal(signal, amplitude, maximumDisplacement, inwardEdges, bands)?.let { heads += it }
    ...
    private fun appendSignal(
        ...
        signal.samples.forEachIndexed { index, sample ->
            ...
            // Mirror a clipped `CENTERED` trace so R stays visible; minor Q/S notches may be clipped.
            val inwardOffset = -outwardOffset
    ...
    internal companion object {
        ...
        private const val SIGNAL_BOUNDS_PADDING = 2
...
internal fun WaveformPainter.paint(
    graphics: Graphics2D,
    request: WaveformPaintRequest,
): WaveformRenderPlan {
