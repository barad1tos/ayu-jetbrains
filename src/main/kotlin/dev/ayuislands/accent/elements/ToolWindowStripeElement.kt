 ...
 * and selected bg sampled independently); [onBackgroundsTinted] wires the
 * first-apply diagnostic.
 *
 * The three background keys are javap-verified against platformVersion 2025.1.
 * `ToolWindow.Button.selectedBackground` is the core New UI stripe-button key
 * and is registered in the bundled `IntelliJPlatform.themeMetadata.json` under
 * `app-client.jar`, so every 2025.1 user has a live LAF entry for it even when
 * the project theme does not.
 *
 * Intensity is read from `AyuIslandsState.chromeTintIntensity`; the per-element
 * revert unconditionally nulls every key the element can write so a toggle
 * flip leaves the stock theme value to re-resolve (Pattern G).
 */
class ToolWindowStripeElement : AbstractChromeElement() {
    ...
    /**
     * Split-fg pick: stripe background and selected-button background are DIFFERENT
     * base colors — at non-trivial intensities their tinted outputs diverge.
     * Sampling the contrast foreground against only one and reusing the pick for
     * the other can drop non-selected stripe icons below WCAG AA.
     */
    ...
    private companion object {
        ...
        /** Foreground key paired with [SELECTED_BACKGROUND_KEY] for the split fg pick. */
        const val SELECTED_FOREGROUND_KEY = "ToolWindow.Button.selectedForeground"

        /** Foreground key paired with [STRIPE_BACKGROUND_KEY] for the split fg pick. */
         ...
         * so runtime class-name string match is the supported lookup path.
         */
