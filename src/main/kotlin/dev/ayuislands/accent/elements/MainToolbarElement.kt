...
/**
 * Tints the main toolbar / title bar.
 *
 * ### Probe gate
 *
 ...
 * Settings row; this class only owns the element-level contract.
 *
 ...
 *
 * `MainToolbar.borderColor` was NOT found in 2025.1 platform metadata or JAR string
 * tables and is therefore intentionally excluded (VERIFY-BEFORE-ASSUMING rule).
 *
...
class MainToolbarElement : AbstractChromeElement() {
    ...
    /** Probe gate — evaluated per `apply` so a mid-session LAF flip is honoured. */
    override val isEnabled: Boolean
        get() = ChromeDecorationsProbe.isCustomHeaderActive()

    private companion object {
        /**
         * FQN of the internal main-toolbar peer — imported as a literal string
         * because the class is package-private and cannot be imported directly.
         * Runtime class-name string match is the supported lookup pattern.
         */
