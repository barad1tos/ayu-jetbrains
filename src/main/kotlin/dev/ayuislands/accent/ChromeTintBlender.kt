...
/**
 * Pure color-math foundation for chrome tinting.
 *
 ...
 * one place.
 *
 * Foreground picking lives in [WcagForeground] — all five chrome elements use
 * [WcagForeground.pickForeground] directly.
 *
...
object ChromeTintBlender {
     ...
     * the stock neutral.
     *
     * Calibrated to `1.0` — the Ayu Cyan reference accent still lands inside
     * the permitted band without damping. Tunable if a future accent palette
     * pushes saturation past the perceptual ceiling.
     */
     ...
     *
     * Pure `B` preservation locks the luma hierarchy but makes the intensity
     * slider imperceptible on dark Mirage bases (`StatusBar.background` sits
     * at ~19% brightness — saturation-only deltas at that low B are below
     * perceptual threshold). Pulling brightness halfway toward the accent
     * (`0.5`) restores slider visibility at intensity ≥ 40 while keeping the
     * relative order of surface luminances at intensity ≤ 40 where the accent
     * pull barely shifts the base. At intensity=100 the output is
     * `bg.B + (accent.B - bg.B) * 0.5` — halfway, not full accent brightness,
     * so the chrome still reads slightly darker than a pure accent fill.
     *
     * Lowering to 0.0 restores B-preservation behaviour (invisible slider on
     * dark bases); raising to 1.0 matches VS Code Peacock (full accent
     ...
     *
     * Output is always opaque (alpha=255) — translucent chrome would bleed
     * through to native OS surfaces. The uniform-hue invariant is locked by
     * [dev.ayuislands.accent.ChromeTintBlenderHueUniformityTest].
     *
    ...
    fun blend(
        ...
        // see the exact stock color when the slider is off. Preserves the
        ...
        // getHSBColor returns an opaque Color; no 4-arg ctor needed.
        // Clamp brightness to [0, 1] to mirror the saturation clamp above.
        // `Color.getHSBColor` passes brightness unchanged into a linear ramp
        // and then into byte-quantisation — out-of-range values from a future
        // tweak to BRIGHTNESS_PULL_RATIO or a base-color extraction edge case
        // would slide into negative/overflow RGB territory without this guard.
