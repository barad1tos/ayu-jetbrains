...
object LicenseChecker {
    ...
    /** Open JetBrains registration/purchase dialog. */
    ...
    private fun isLicenseServerStampValid(serverStamp: String): Boolean {
        try {
            ...
            val certBase64 = parts[4]
            val intermediateCertsBase64 = if (parts.size > 5) parts.subList(5, parts.size) else emptyList()
            ...
            // Timestamp must be within validity period
    ...
    private fun createCertificate(
        ...
        val intermediateCerts = intermediateCertsBytes.map { bytes ->
            factory.generateCertificate(ByteArrayInputStream(bytes)) as X509Certificate
        }

        // Build trust anchors from root certificates
        val trustAnchors = ROOT_CERTIFICATES.map { rootCertPem ->
            val rootCert = factory.generateCertificate(
