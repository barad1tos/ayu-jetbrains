...
object SyntaxIntensityApplicator {
    ...
    private fun resolveFallbackAttributes(
        ...
        while (true) {
            val currentFallbackName = fallbackName ?: break
            if (!visited.add(currentFallbackName)) break
            ...
            fallbackName = sources.fallbacks[currentFallbackName] ?: fallback.fallbackAttributeKey?.externalName
        }
        return null
    }
    ...
    private const val EMPHASIZE_DECLARATIONS_SATURATION_DELTA = 0.12f
    private const val EMPHASIZE_DECLARATIONS_LIGHTNESS_DELTA = 0.14f
