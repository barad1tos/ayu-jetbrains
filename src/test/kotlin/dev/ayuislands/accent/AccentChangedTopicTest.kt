...
class AccentChangedTopicTest {
    ...
    fun `topic exists with stable display name`() {
        ...
        assertEquals(
            "Ayu Accent Changed",
            topic.displayName,
            "Topic display name must be the contractual string so bus diagnostics stay grep-able",
        )
    }
    ...
    fun `AccentChangeListener is a fun interface with a single accentChanged method`() {
        ...
        // Kotlin mangles the JVM name of functions that take a value class
        // parameter (`AccentHex` → `accentChanged-Czfobf0`). The Kotlin
        ...
        // Payload signature: (project, AccentHex erased to String, source) —
        // load-bearing for the topic contract. The hex parameter is the
        // validated [AccentHex] value class. JVM bytecode erases value classes
        // to their underlying type (`String`), but the mangled method name
        // (`accentChanged-Czfobf0`, asserted above) carries the value-class
        // fingerprint and prevents an accidental rewind to a raw-String signature.
