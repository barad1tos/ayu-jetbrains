...
class TypingCadenceTest {
    @Test
    fun `single keystroke measures no rate and keeps resting speed`() {
        ...
        assertEquals(1f, cadence.targetSpeed(nowMs = 1_000L), 0.001f)
    }
    ...
    fun `gap beyond reset threshold restarts measurement`() {
        ...
        assertEquals(0f, cadence.strokesPerSecond, 0.001f)
        assertEquals(1f, cadence.targetSpeed(nowMs = 100L + TypingCadence.CADENCE_RESET_MS + 1L), 0.001f)
    }

    @Test
    fun `target speed rises smoothly and clamps at the maximum`() {
        var halfRate = TypingCadence()
        halfRate = halfRate.keystroke(nowMs = 0L)
        halfRate = halfRate.keystroke(nowMs = 100L)
        assertEquals(1.75f, halfRate.targetSpeed(nowMs = 100L), 0.001f)
        ...
        assertEquals(
            TypingCadence.MAX_SPEED_MULTIPLIER,
            fullRate.targetSpeed(nowMs = 20 * 50L),
            0.01f,
        )
    }
