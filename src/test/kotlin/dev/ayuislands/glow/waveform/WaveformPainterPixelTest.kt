...
class WaveformPainterPixelTest {
    ...
    @Test
    fun `signal bounds contain every pixel changed above the shared base`() {
        val config =
            WaveformConfig(
                amplitude = 16,
                intensity = 100,
                traceLength = MIN_TRACE_LENGTH,
            )
        val active = render(frame(config, center = 0f))
        val base = renderSolidBase()
        val signalBounds = assertNotNull(active.plan.signalBounds)

        assertEquals(0, changedOutside(base, active.image, signalBounds))
        assertTrue(pixelDifference(base, active.image) > MIN_PIXEL_DIFFERENCE)
        assertTrue(
            signalBounds.width.toLong() * signalBounds.height < bounds.width.toLong() * bounds.height / 2,
            "dirty bounds must stay local to the compact ECG trace: $signalBounds",
        )
    }

    @Test
    fun `union of consecutive signal bounds contains every changed pixel`() {
        val config =
            WaveformConfig(
                amplitude = 16,
                intensity = 100,
        ...
        val first = render(frame(config, center = 0f))
        val second = render(frame(config, center = 80f))
        val firstBounds = assertNotNull(first.plan.signalBounds)
        val secondBounds = assertNotNull(second.plan.signalBounds)
        val dirtyBounds = firstBounds.union(secondBounds)

        assertTrue(pixelDifference(first.image, second.image) > MIN_PIXEL_DIFFERENCE)
        assertEquals(0, changedOutside(first.image, second.image, dirtyBounds))
    }
    ...
    fun `R peak changes production waveform pixels`() {
        ...
        assertSame(flat.plan.track, beat.plan.track, "unchanged geometry must reuse the sampled perimeter")
    }
    ...
    fun `editor perimeter loop starts from the two thirds top anchor`() {
        for (direction in TravelDirection.entries) {
            ...
            val topAnchorBand =
                Rectangle(
                    (anchor.x - started.plan.track.signalSpan / 2f).roundToInt().coerceAtLeast(0),
                    0,
                    0,
                    HEIGHT / 3,
                ).also { region ->
                    region.width =
                        started.plan.track.signalSpan
                            .roundToInt()
            ...
            assertTrue(
                ...
                "$direction perimeter loop must start at the top anchor",
            )
    ...
    @Test
    fun `editor resting waveform ignores loop direction`() {
        val clockwise =
            render(
                WaveformFrame(
                    direction = TravelDirection.CLOCKWISE,
    ...
    fun `single blocked edge keeps the other edge outward`() {
        ...
        val beat =
            render(
                frame(
                    config,
                    rightSample.distance - flat.plan.track.signalAnchorDistance,
                    BeatMorphology.standard(),
                ),
                inwardEdges = setOf(WaveformEdge.TOP),
            )
    ...
    fun `single blocked edge changes only its corner component`() {
        ...
        val beat =
            render(
                frame(
                    config,
                    corner.distance - flat.plan.track.signalAnchorDistance,
                    BeatMorphology.standard(),
                ),
                inwardEdges = setOf(WaveformEdge.TOP),
            )
    ...
    fun `clipped top edge renders a full ECG peak inside content`() {
        ...
        val anchor = rendered.plan.track.sampleNearest(rendered.plan.track.signalAnchorDistance)
        val visibleClip =
            Rectangle(
                (anchor.x - FLASH_HALF_WIDTH).roundToInt(),
                contentBounds.y,
    ...
    @Test
    fun `idle perimeter trace preserves configured peak range while typing changes luminance`() {
        ...
        val trackLength = renderEditorScale(activated).plan.track.length
        ...
        val anchor = idle.plan.track.sampleNearest(idle.plan.track.signalAnchorDistance)
        ...
        assertTrue(
            ...
            "perimeter trace amplitude must not collapse while idle: height=$idlePeakHeight signal=$idleBounds",
        )
        assertTrue(
            activePeakHeight >= config.amplitude * MIN_ACTIVE_HEIGHT_FRACTION,
            "active perimeter trace amplitude must remain full sized: height=$activePeakHeight signal=$activeBounds",
        )
        assertTrue(
        ...
        assertTrue(
            ...
            "typing must brighten the perimeter trace without changing its amplitude",
        )
        assertEquals(
            DEFAULT_TRACE_LENGTH.toFloat(),
            idle.plan.track.signalSpan * TRACE_PHASE_SPAN,
            0.1f,
        )
    }

    @Test
    fun `ECG trace evolves independently of perimeter travel`() {
    ...
    @Test
    fun `track cache repositions a centered perimeter trace when direction changes`() {
        val clockwise =
            renderEditorScale(
                WaveformFrame(
                    direction = TravelDirection.CLOCKWISE,
        ...
        assertTrue(
            abs(clockwise.plan.track.signalAnchorDistance - counterClockwise.plan.track.signalAnchorDistance) > 1f,
        )
    }
    ...
    @Test
    fun `maximum density renders four times as many R peaks without widening the trace`() {
        val morphology = BeatMorphology.standard()
        val defaultConfig =
            WaveformConfig(
                amplitude = MAX_WAVEFORM_AMPLITUDE,
        ...
        assertEquals(default.plan.track.signalSpan, maximum.plan.track.signalSpan, 0.001f)
        assertEquals(expectedRPeaks(defaultFrame), defaultPeaks)
        assertEquals(
            defaultPeaks * MAX_TRACE_DENSITY,
            maximumPeaks,
            "the rendered trace must contain four times as many distinct R peaks",
        )
    }
    ...
    fun `centered waveform straddles the middle of the solid glow band`() {
        ...
        val outsideBaseline =
            outside.plan.track.samples
                .first { it.normalY < -0.999f }
                .y
                .roundToInt()
        val centeredBaseline =
            centered.plan.track.samples
                .first { it.normalY < -0.999f }
                .y
                .roundToInt()
    ...
    private fun changedOutside(
        first: BufferedImage,
        second: BufferedImage,
        region: Rectangle,
    ): Int =
        (0 until first.height).sumOf { y ->
            (0 until first.width).count { x ->
                !region.contains(x, y) && first.getRGB(x, y) != second.getRGB(x, y)
            }
        }
