...
class EditorTabGeometryTest {
    ...
    @Test
    fun `calculateEditorOverlayBounds uses full host when tabs have no visible labels`() {
        val host = JPanel(null)
        host.setSize(800, 600)
        val editorTabs = SelectionEditorTabsFake()
        editorTabs.setBounds(0, 0, 800, 600)
        ...
        val hiddenLabel = MockTabLabel()
        hiddenLabel.setBounds(0, 0, 120, 25)
        hiddenLabel.isVisible = false
        editorTabs.add(hiddenLabel)
        editorTabs.labelForTest = hiddenLabel

        val bounds = EditorTabGeometry.calculateEditorOverlayBounds(host)

        assertEquals(Rectangle(0, 0, 800, 600), bounds)
    }
